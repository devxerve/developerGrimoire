...
```dataviewjs
// -----------------------------------------------
//  DASHBOARD ZEN-OS  —  Professional Edition
// -----------------------------------------------
...
const DEFAULT_LEFT_ORDER = ["clock","objective","system"];
let leftOrder = JSON.parse(localStorage.getItem("zos-left-order") || "null");
if(!leftOrder || leftOrder.length !== DEFAULT_LEFT_ORDER.length) leftOrder = [...DEFAULT_LEFT_ORDER];

const RIGHT_DEFAULT = ["pipeline","ctf","links"];
let rightOrder = JSON.parse(localStorage.getItem("zos-right-order") || "null");
// Remove wizard from any saved order
...
function timerColor(secs) {
    if(secs<=0)     return "rgba(255,255,255,0.25)";
    if(secs<3600)   return "#ff5555";
    if(secs<86400)  return "#ff8c00";
    if(secs<259200) return "#ffc800";
...
function modal({title,type="text",value="",placeholder="",options=[],selected=[],onSave,onDelete}){
    ...
    const mo=ov.createDiv({cls:"zos-modal"});
    mo.createDiv({cls:"zos-modal-title",text:title});
    const cl=()=>ov.remove();
    ov.onclick=e=>{if(e.target===ov)cl();};
    if(type==="text"){
        const inp=mo.createEl("input",{cls:"zos-modal-input",attr:{type:"text",value,placeholder:placeholder||"Type here..."}});
        ...
        const ta=mo.createEl("textarea",{attr:{rows:"4",placeholder,style:"width:100%;background:#1a1a1a;border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:14px;color:#fff;font-family:var(--zos-font-main);font-size:0.9em;outline:none;margin-bottom:20px;resize:vertical;box-sizing:border-box;"}});
        ...
        options.forEach(o=>{
            const b=gc.createEl("button",{cls:"zos-modal-btn",text:o.label,attr:{style:"background:rgba(255,255,255,0.05);color:white;border:1px solid rgba(255,255,255,0.1);text-align:left;padding:14px;font-weight:600;"}});
            b.onclick=()=>{onSave(o.value);cl();};
            b.onmouseenter=()=>{b.style.background="rgba(var(--zos-accent-rgb),0.1)";b.style.borderColor="var(--zos-accent)";};
            b.onmouseleave=()=>{b.style.background="rgba(255,255,255,0.05)";b.style.borderColor="rgba(255,255,255,0.1)";};
        });
        mo.createEl("button",{cls:"zos-modal-btn zos-btn-cancel",text:"Close",attr:{style:"width:100%"}}).onclick=cl;
    }else if(type==="multicheck"){
        const srch=mo.createEl("input",{attr:{placeholder:"Search calendars...",style:"width:100%;background:#1a1a1a;border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:10px 14px;color:#fff;font-size:0.85em;outline:none;margin-bottom:12px;box-sizing:border-box;"}});
        ...
        const renderList=(f="")=>{
            ...
            options.filter(o=>o.label.toLowerCase().includes(f.toLowerCase())).forEach(o=>{
                const r=list.createDiv({attr:{style:"display:flex;align-items:center;gap:10px;padding:10px 12px;background:rgba(255,255,255,0.03);border-radius:10px;cursor:pointer;border:1px solid transparent;transition:all 0.15s;"}});
                const on=ck.has(o.value);
                const box=r.createDiv({attr:{style:`width:16px;height:16px;border-radius:4px;border:2px solid ${on?"var(--zos-accent)":"rgba(255,255,255,0.2)"};background:${on?"var(--zos-accent)":"transparent"};flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:10px;`}});
                if(on)box.innerText="✓";
                r.createDiv({text:o.label,attr:{style:"font-size:0.84em;font-weight:600;"}});
                r.onclick=()=>{
                    if(ck.has(o.value)){ck.delete(o.value);box.style.background="transparent";box.style.borderColor="rgba(255,255,255,0.2)";box.innerText="";r.style.borderColor="transparent";}
        ...
        mo.createEl("button",{cls:"zos-modal-btn",text:"Show All",attr:{style:"width:100%;margin-top:8px;background:rgba(255,255,255,0.03);color:rgba(255,255,255,0.4);border:1px dashed rgba(255,255,255,0.08);"}}).onclick=()=>{onSave([]);cl();};
    }
}
...
function openAskAIAnswerModal(question,answer,providerLabel="AI"){
    ...
    const ov=document.body.createDiv({cls:"zos-modal-overlay"});
    const mo=ov.createDiv({cls:"zos-modal",attr:{style:"width:min(780px,92vw);max-height:88vh;display:flex;flex-direction:column;padding:0;overflow:hidden;"}});
    const close=()=>ov.remove();
    ov.onclick=e=>{if(e.target===ov)close();};

    // -- HEADER ----------------
    const hdr=mo.createDiv({attr:{style:"display:flex;align-items:center;justify-content:space-between;padding:16px 20px 12px;border-bottom:1px solid rgba(255,255,255,0.06);flex-shrink:0;"}});
    hdr.createDiv({attr:{style:"font-size:0.72em;font-weight:800;letter-spacing:2px;opacity:0.5;"}}).innerText="AI CHAT";
    ...
    const closeBtn=hdr.createDiv({attr:{style:"font-size:0.8em;opacity:0.3;cursor:pointer;padding:4px 8px;border-radius:6px;transition:all 0.2s;"}});
    closeBtn.innerText="✕";
    closeBtn.onmouseenter=()=>{closeBtn.style.opacity="1";closeBtn.style.background="rgba(255,85,85,0.15)";closeBtn.style.color="#ff5555";};
    closeBtn.onmouseleave=()=>{closeBtn.style.opacity="0.3";closeBtn.style.background="";closeBtn.style.color="";};
    closeBtn.onclick=close;

    // -- MESSAGES AREA -------------
    const msgArea=mo.createDiv({attr:{style:"flex:1;overflow-y:auto;padding:16px 20px;display:flex;flex-direction:column;gap:14px;min-height:0;"}});
    ...
    const addTypingIndicator=()=>{
        ...
        const bubble=wrap.createDiv({attr:{style:"padding:10px 16px;border-radius:14px 14px 14px 4px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);display:flex;gap:5px;align-items:center;"}});
        [0,0.2,0.4].forEach(delay=>{
            const dot=bubble.createDiv({attr:{style:"width:6px;height:6px;border-radius:50%;background:rgba(255,255,255,0.4);animation:zos-dot-pulse 1.2s ease-in-out "+delay+"s infinite;"}});
        });
    ...
    // -- INPUT AREA --------------
    const inputArea=mo.createDiv({attr:{style:"flex-shrink:0;padding:12px 16px;border-top:1px solid rgba(255,255,255,0.06);"}});
    const inputRow=inputArea.createDiv({attr:{style:"display:flex;align-items:center;gap:8px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:6px 6px 6px 14px;transition:all 0.2s;"}});
    const chatInput=inputRow.createEl("input",{attr:{type:"text",placeholder:"Ask a follow-up question...",style:"flex:1;background:transparent;border:none;color:#fff;font-size:0.8em;outline:none;font-family:var(--zos-font-main);padding:6px 0;"}});
    ...
    chatInput.onblur=()=>{inputRow.style.borderColor="rgba(255,255,255,0.08)";};
...
const heroTitleEl=hero.createEl("h1",{text:bannerTitle,attr:{style:"font-size:3.5em;font-weight:900;letter-spacing:-2px;margin-bottom:5px;background:linear-gradient(to right,#fff,rgba(255,255,255,0.4));-webkit-background-clip:text;-webkit-text-fill-color:transparent;"}});
...
// -- AI BAR LABEL --------------
const askLabel=askWrap.createDiv({attr:{style:"display:flex;align-items:center;gap:6px;margin-bottom:8px;"}});
askLabel.createDiv({attr:{style:"width:6px;height:6px;border-radius:50%;background:var(--zos-accent);box-shadow:0 0 8px rgba(var(--zos-accent-rgb),0.8);animation:zos-pulse 2s ease-in-out infinite;"}});
askLabel.createDiv({attr:{style:"font-size:0.58em;font-weight:800;letter-spacing:2.5px;opacity:0.35;color:#fff;"}}).innerText="AI ASSISTANT";

// -- AI BAR CONTAINER --------------
const askBar=askWrap.createDiv({attr:{style:"display:flex;align-items:center;gap:0;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:16px;overflow:hidden;transition:all 0.3s ease;box-shadow:0 4px 24px rgba(0,0,0,0.2);"}});
...
// -- INPUT -----------------
const askInput=askBar.createEl("input",{attr:{type:"text",placeholder:"Ask anything — assignments, concepts, deadlines...",style:"flex:1;background:transparent;border:none;padding:13px 4px;color:#fff;font-size:0.8em;outline:none;font-family:var(--zos-font-main);letter-spacing:0.2px;"}});

// -- DIVIDER -----------------
askBar.createDiv({attr:{style:"width:1px;height:20px;background:rgba(255,255,255,0.08);flex-shrink:0;"}});

// -- SEND BUTTON ---------------
const askBtn=askBar.createEl("button",{attr:{style:"display:flex;align-items:center;gap:6px;padding:10px 18px;background:transparent;color:rgba(255,255,255,0.35);border:none;cursor:pointer;font-size:0.72em;font-weight:700;letter-spacing:1px;transition:all 0.2s;white-space:nowrap;font-family:var(--zos-font-main);"}});
...
[
    ...
    const chip=quickWrap.createEl("button",{attr:{style:"font-size:0.62em;font-weight:600;padding:4px 10px;border-radius:20px;background:rgba(255,255,255,0.04);color:rgba(255,255,255,0.35);border:1px solid rgba(255,255,255,0.07);cursor:pointer;transition:all 0.2s;letter-spacing:0.3px;"}});
    ...
    chip.onmouseleave=()=>{chip.style.background="rgba(255,255,255,0.04)";chip.style.color="rgba(255,255,255,0.35)";chip.style.borderColor="rgba(255,255,255,0.07)";};
    chip.onclick=()=>{
...
askInput.onblur=()=>{
    askBar.style.borderColor="rgba(255,255,255,0.08)";
    askBar.style.boxShadow="0 4px 24px rgba(0,0,0,0.2)";
    askIcon.style.opacity="0.3";
    askIcon.style.color="";
    askBtn.style.color="rgba(255,255,255,0.35)";
};
askBtn.onmouseenter=()=>{askBtn.style.background="rgba(var(--zos-accent-rgb),0.1)";askBtn.style.color="var(--zos-accent)";};
askBtn.onmouseleave=()=>{askBtn.style.background="transparent";askBtn.style.color=document.activeElement===askInput?"var(--zos-accent)":"rgba(255,255,255,0.35)";};
askBtn.onclick=()=>{
...
// -- MOTIVATIONAL QUOTE --------------
const quoteEl=hero.createDiv({attr:{style:"opacity:0.6;font-size:1.1em;letter-spacing:0.5px;font-style:italic;max-width:700px;"}});
...
refreshQuote(false);
lastSyncEl=hero.createDiv({attr:{style:"font-size:0.65em;opacity:0.25;margin-top:6px;font-family:var(--zos-font-mono);"}});

const bsBtn=header.createDiv({cls:"zos-banner-settings-btn"});
try{setIcon(bsBtn,"settings");}catch(e){bsBtn.innerText="";}
bsBtn.onclick=()=>modal({title:"Edit Banner",type:"grid",options:[{label:"✎  Edit Title",value:"title"},{label:"✎  Edit Subtitle",value:"subtitle"},{label:"↻  Refresh quote",value:"quote"}],onSave:c=>{
    ...
    else if(c==="subtitle")modal({title:"Subtitle",value:bannerSubtitle,placeholder:"E.g.: Welcome back",onSave:v=>{bannerSubtitle=v.trim();save("zos-banner-subtitle",bannerSubtitle);heroSubEl.innerText=bannerSubtitle||`${greeting()} — Welcome back. The entire system is ready.`;}});
...
syncBtn.onclick=()=>{fetchAndRender();fetchCTFs();refreshQuote(true);};
...
function renderHeaderStats(){
    ...
    [{v:dv.pages().length,l:"Notes",i:"files"},{v:pending,l:"Pending",i:"clock"}].forEach(s=>{
        ...
        const ic=tr.createDiv();try{setIcon(ic,s.i);}catch(e){ic.innerText="";}
...
const leftCol=grid.createDiv({cls:"zos-col-grid zos-left-col",attr:{style:"grid-column:span 4;"}});
const rightCol=grid.createDiv({cls:"zos-col-grid zos-right-col",attr:{style:"grid-column:span 8;"}});
...
function buildClockCard(){
    ...
    const cClock=w.createDiv({cls:"zos-card zos-clock-card"});
    ...
    const mkBar=(label,color,last)=>{
        ...
        const bg=cClock.createDiv({attr:{style:`height:5px;background:rgba(255,255,255,0.05);border-radius:3px;overflow:hidden;${last?"":"margin-bottom:18px;"}`}});
...
function buildObjectiveCard(){
    ...
    const cFocus=w.createDiv({cls:"zos-card"});
...
function buildSystemCard(){
    ...
    const cAct=w.createDiv({cls:"zos-card"});
    ...
    const pomBar=cAct.createDiv({attr:{style:"display:none;margin-top:16px;padding:14px;background:rgba(255,255,255,0.03);border-radius:14px;border:1px solid rgba(255,255,255,0.06);"}});
    ...
    const pomReset=pomBtnRow.createEl("button",{attr:{style:"font-size:0.75em;font-weight:700;padding:8px 12px;border-radius:10px;background:rgba(255,255,255,0.04);color:rgba(255,255,255,0.4);border:1px solid rgba(255,255,255,0.08);cursor:pointer;"}});
    pomReset.innerText="↺";
    ...
    const pomSetPhase=(i)=>{
        ...
        pomPhaseRow.querySelectorAll("button").forEach((b,j)=>{
            b.style.background=j===i?"rgba(var(--zos-accent-rgb),0.15)":"rgba(255,255,255,0.04)";
            b.style.color=j===i?"var(--zos-accent)":"rgba(255,255,255,0.4)";
        });
    };
    pomPhases.forEach((p,i)=>{
        const tb=pomPhaseRow.createEl("button",{attr:{style:`flex:1;font-size:0.62em;font-weight:700;padding:4px 6px;border-radius:8px;cursor:pointer;transition:all 0.2s;border:1px solid rgba(255,255,255,0.06);background:${i===0?"rgba(var(--zos-accent-rgb),0.15)":"rgba(255,255,255,0.04)"};color:${i===0?"var(--zos-accent)":"rgba(255,255,255,0.4)"};`}});
    ...
    [{n:"Search",icon:"search",em:"search",cmd:"__search__"},{n:"Stats",icon:"bar-chart-2",em:"bar-chart-2",cmd:"__stats__"},{n:"Pomodoro",icon:"timer",em:"timer",cmd:"__pomodoro__"},{n:"New Note",icon:"plus-circle",em:"plus-circle",cmd:"__newnote__"}].forEach(op=>{
        ...
        if(op.cmd==="__search__"){
            btn.onclick=()=>{
                const ov=document.body.createDiv({cls:"zos-modal-overlay"});
                const mo=ov.createDiv({cls:"zos-modal",attr:{style:"width:520px;max-height:85vh;display:flex;flex-direction:column;"}});
                ov.onclick=e=>{if(e.target===ov)ov.remove();};
                mo.createDiv({cls:"zos-modal-title",text:"SEARCH VAULT"});
                const sub=mo.createDiv({attr:{style:"font-size:0.72em;opacity:0.5;margin-bottom:12px;"}});
                ...
                const resultsEl=mo.createDiv({attr:{style:"flex:1;overflow-y:auto;max-height:360px;min-height:120px;border:1px solid rgba(255,255,255,0.06);border-radius:10px;padding:10px;background:rgba(0,0,0,0.2);"}});
                ...
                const runSearch=async()=>{
                    ...
                    hits.forEach(({file,path,snippet})=>{
                        const item=resultsEl.createDiv({attr:{style:"padding:10px 12px;margin-bottom:6px;background:rgba(255,255,255,0.04);border-radius:8px;border:1px solid rgba(255,255,255,0.06);cursor:pointer;transition:background 0.15s,border-color 0.15s;"}});
                        ...
                        item.onmouseleave=()=>{item.style.background="rgba(255,255,255,0.04)";item.style.borderColor="rgba(255,255,255,0.06)";};
                    });
                };
            ...
            btn.onclick=()=>{
                ...
                const mo=ov.createDiv({cls:"zos-modal"});
                ov.onclick=e=>{if(e.target===ov)ov.remove();};
                mo.createDiv({cls:"zos-modal-title",text:"VAULT STATS"});
                [{l:"Total Notes",v:notes.length},{l:"Total Files",v:files.length},{l:"Folders",v:folders}].forEach(s=>{
                    const row=mo.createDiv({attr:{style:"display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.05);"}});
            ...
            btn.onclick=async()=>{
                ...
                const ov=document.body.createDiv({cls:"zos-modal-overlay"});
                const mo=ov.createDiv({cls:"zos-modal",attr:{style:"width:480px;max-height:80vh;display:flex;flex-direction:column;"}});
                ov.onclick=e=>{if(e.target===ov)ov.remove();};
                mo.createDiv({cls:"zos-modal-title",text:"NEW NOTE"});
                ...
                nameInp.focus();
                const breadcrumbEl=mo.createDiv({attr:{style:"display:flex;align-items:center;gap:4px;flex-wrap:wrap;padding:8px 12px;background:rgba(255,255,255,0.03);border-radius:10px;margin-bottom:10px;border:1px solid rgba(255,255,255,0.06);min-height:34px;"}});
                ...
                const newFolderInp=newFolderRow.createEl("input",{attr:{type:"text",placeholder:"New folder name...",style:"flex:1;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:8px 12px;color:#fff;font-size:0.8em;outline:none;"}});
                ...
                newFolderInp.onblur=()=>{newFolderInp.style.borderColor="rgba(255,255,255,0.08)";};
                ...
                const renderBreadcrumb=()=>{
                    ...
                    const rc=breadcrumbEl.createDiv({attr:{style:"font-size:0.72em;font-weight:500;color:rgba(255,255,255,0.4);cursor:pointer;transition:color 0.15s;"}});
                    rc.innerText="Root";rc.onclick=()=>{currentPath="/";renderFolders();};
                    rc.onmouseenter=()=>{rc.style.color="rgba(255,255,255,0.7)";};rc.onmouseleave=()=>{rc.style.color="rgba(255,255,255,0.4)";};
                    breadcrumbEl.createDiv({attr:{style:"font-size:0.7em;opacity:0.3;"}}).innerText="›";
                    parts.forEach((part,i)=>{
                        const isLast=i===parts.length-1;
                        const crumb=breadcrumbEl.createDiv({attr:{style:`font-size:0.72em;font-weight:${isLast?"700":"500"};color:${isLast?"var(--zos-accent)":"rgba(255,255,255,0.4)"};${isLast?"":"cursor:pointer;"}transition:color 0.15s;white-space:nowrap;`}});
                        crumb.innerText=part;
                        if(!isLast){crumb.onclick=()=>{currentPath=parts.slice(0,i+1).join("/");renderFolders();};crumb.onmouseenter=()=>{crumb.style.color="rgba(255,255,255,0.7)";};crumb.onmouseleave=()=>{crumb.style.color="rgba(255,255,255,0.4)";};breadcrumbEl.createDiv({attr:{style:"font-size:0.7em;opacity:0.3;"}}).innerText="›";}
                    });
                };
                const renderFolders=()=>{
                    ...
                    if(currentPath!=="/"){
                        const upBtn=folderList.createDiv({attr:{style:"display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:10px;cursor:pointer;transition:background 0.15s;color:rgba(255,255,255,0.4);font-size:0.82em;"}});
                        upBtn.createDiv({attr:{style:"width:18px;text-align:center;"}}).innerText="↑";
                        upBtn.createDiv({text:".. (go up)"});
                        upBtn.onmouseenter=()=>{upBtn.style.background="rgba(255,255,255,0.04)";};upBtn.onmouseleave=()=>{upBtn.style.background="";};
...
try{setIcon(folderIcon,"folder");}catch(e){folderIcon.innerText="▶";}
                        ...
                        row.onmouseenter=()=>{row.style.background="rgba(255,255,255,0.05)";row.style.borderColor="rgba(255,255,255,0.07)";};
...
function buildPipelineCard(){
    ...
    const cTasks=w.createDiv({cls:"zos-card"});
    ...
    calNavL.onmouseenter=()=>{calNavL.style.opacity="1";calNavL.style.background="rgba(255,255,255,0.05)";};calNavL.onmouseleave=()=>{calNavL.style.opacity="0.5";calNavL.style.background="";};
    ...
    calNavR.onmouseenter=()=>{calNavR.style.opacity="1";calNavR.style.background="rgba(255,255,255,0.05)";};calNavR.onmouseleave=()=>{calNavR.style.opacity="0.5";calNavR.style.background="";};
...
function buildCTFCard(){
    ...
    const cCTF=w.createDiv({cls:"zos-card"});
...
function renderCTFGroup(label,color,evs,now,filteredEvs){
    ...
    const hdr=ctfListEl.createDiv({attr:{style:"display:flex;align-items:center;gap:8px;margin:8px 0 4px;padding-bottom:7px;border-bottom:1px solid rgba(255,255,255,0.05);"}});
    ...
    evs.forEach(ev=>{
        ...
        const card=ctfListEl.createDiv({attr:{style:`padding:14px 16px;background:rgba(255,255,255,0.025);border-radius:14px;border:1px solid ${isOngoing?"rgba(0,255,136,0.15)":"rgba(255,255,255,0.06)"};transition:all 0.2s;cursor:pointer;position:relative;overflow:hidden;`}});
        ...
        card.onmouseenter=()=>{card.style.background="rgba(255,255,255,0.04)";card.style.borderColor=isOngoing?"rgba(0,255,136,0.3)":"rgba(255,255,255,0.1)";};
        card.onmouseleave=()=>{card.style.background="rgba(255,255,255,0.025)";card.style.borderColor=isOngoing?"rgba(0,255,136,0.15)":"rgba(255,255,255,0.06)";};
...
card.setAttribute("data-ctf-card","1");
    ...
    const mo2=ov2.createDiv({cls:"zos-modal",attr:{style:"width:500px;max-height:85vh;overflow-y:auto;"}});
    const cl2=()=>ov2.remove();
    ov2.onclick=e2=>{if(e2.target===ov2)cl2();};
    mo2.createDiv({cls:"zos-modal-title",text:"CTF DETAILS"});
    ...
    if(full.format||ev.format)statusRow.createDiv({attr:{style:"font-size:0.65em;opacity:0.45;font-weight:600;padding:3px 9px;border-radius:8px;background:rgba(255,255,255,0.04);"}}).innerText=full.format||ev.format;
    if((full.weight||0)>0)statusRow.createDiv({attr:{style:"font-size:0.65em;opacity:0.45;font-weight:600;padding:3px 9px;border-radius:8px;background:rgba(255,255,255,0.04);"}}).innerText=`Weight: ${full.weight}`;

    // Dates
    const dateBox=mo2.createDiv({attr:{style:"background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px;padding:12px 14px;margin-bottom:14px;display:grid;grid-template-columns:1fr 1fr;gap:8px;"}});
    ...
    if(full.url||full.ctftime_url){
        const urlBox=mo2.createDiv({attr:{style:"background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px;padding:10px 14px;margin-bottom:14px;"}});
    ...
    if(desc){
        ...
        descBox.createDiv({attr:{style:"font-size:0.8em;opacity:0.65;line-height:1.7;background:rgba(255,255,255,0.03);border-radius:12px;padding:12px;border:1px solid rgba(255,255,255,0.05);max-height:140px;overflow-y:auto;white-space:pre-wrap;"}}).innerText=desc;
    }
    ...
    if(full.prizes){
        ...
        prizeBox.createDiv({attr:{style:"font-size:0.8em;opacity:0.65;line-height:1.7;background:rgba(255,255,255,0.03);border-radius:12px;padding:12px;border:1px solid rgba(255,255,255,0.05);"}}).innerText=full.prizes.replace(/<[^>]+>/g," ").trim();
    }
    ...
    if(full.participants)statsRow.createDiv({attr:{style:"font-size:0.65em;opacity:0.45;padding:3px 9px;border-radius:8px;background:rgba(255,255,255,0.04);"}}).innerText=`👥 ${full.participants} participants`;
    if(full.is_votable_now)statsRow.createDiv({attr:{style:"font-size:0.65em;padding:3px 9px;border-radius:8px;background:rgba(0,255,136,0.08);color:#00ff88;"}}).innerText="✓ Votable";
    if(full.individual)statsRow.createDiv({attr:{style:"font-size:0.65em;opacity:0.45;padding:3px 9px;border-radius:8px;background:rgba(255,255,255,0.04);"}}).innerText="👤 Individual";

    // Buttons
    const btnRow=mo2.createDiv({attr:{style:"display:flex;gap:8px;margin-bottom:8px;"}});
    const regKey=`zos-ctf-reg-${ev.ctf_id}`;
    const isRegNow=!!localStorage.getItem(regKey);
    const regBtnModal=btnRow.createEl("button",{attr:{style:`flex:1;font-size:0.75em;font-weight:700;padding:9px;border-radius:9px;background:${isRegNow?"rgba(0,255,136,0.12)":"rgba(255,255,255,0.05)"};color:${isRegNow?"#00ff88":"rgba(255,255,255,0.5)"};border:1px solid ${isRegNow?"rgba(0,255,136,0.25)":"rgba(255,255,255,0.08)"};cursor:pointer;transition:all 0.2s;`}});
    ...
    if(full.ctftime_url||ev.ctftime_url){
        const ct=btnRow.createEl("button",{attr:{style:`flex:1;font-size:0.75em;font-weight:700;padding:9px;border-radius:9px;background:rgba(255,255,255,0.04);color:rgba(255,255,255,0.5);border:1px solid rgba(255,255,255,0.08);cursor:pointer;`}});
        ct.innerText="CTFtime ↗";
        ct.onclick=()=>window.open(full.ctftime_url||ev.ctftime_url,"_blank");
    }
    mo2.createEl("button",{cls:"zos-modal-btn zos-btn-cancel",text:"Close",attr:{style:"width:100%;"}}).onclick=cl2;
};
    });
}
...
function buildLinksCard(){
    ...
    const w = makeRightCard("links");
    w.classList.add("zos-resizable-card");
    attachResizeHandle(w, "links", () => rightCol);
    const cLinks = w.createDiv({cls: "zos-card"});
    ...
    dragHandle.setAttribute("data-right-drag", "true");
    ...
    // -- LINKS GRID ----------------
    const linksGrid = cLinks.createDiv({attr: {style: "display:flex;flex-direction:column;gap:6px;max-height:380px;overflow-y:auto;padding-right:4px;"}});
    ...
    const row = linksGrid.createDiv({attr: {
        ...
        style: "display:flex;align-items:center;gap:12px;padding:9px 12px;background:rgba(255,255,255,0.025);border:1px solid rgba(255,255,255,0.06);border-radius:12px;cursor:pointer;transition:all 0.2s;position:relative;user-select:none;"
    }});

    // Drag handle
    const dragGrip = row.createDiv({attr: {style: "font-size:0.75em;opacity:0.2;cursor:grab;flex-shrink:0;letter-spacing:-1px;transition:opacity 0.15s;"}});
    dragGrip.innerText = "⠿";

    // Favicon / emoji icon
    const iconWrap = row.createDiv({attr: {style: "width:28px;height:28px;border-radius:8px;background:rgba(255,255,255,0.05);display:flex;align-items:center;justify-content:center;overflow:hidden;flex-shrink:0;"}});
    if (link.emoji) {
    ...
    if (!activeCategory) {
        ...
        catRow.createDiv({attr: {style: "font-size:0.65em;opacity:0.35;font-weight:600;"}}).innerText = link.category || "General";
    }

    // Edit btn
    const editBtn = row.createDiv({attr: {style: "font-size:0.65em;opacity:0;padding:3px 8px;border-radius:6px;background:rgba(255,255,255,0.06);cursor:pointer;transition:opacity 0.15s;flex-shrink:0;border:1px solid rgba(255,255,255,0.08);"}});
    editBtn.innerText = "✎";
    ...
    const openLinkModal = (editIdx = -1) => {
        ...
        const field = (label, placeholder, value="") => {
            ...
            wrap.createDiv({attr: {style: "font-size:0.62em;font-weight:700;letter-spacing:1.5px;opacity:0.35;margin-bottom:5px;text-transform:uppercase;"}}).innerText = label;
...
function openMaterialModal(title, url, type, mime=""){
    const ov = document.body.createDiv({cls:"zos-modal-overlay"});
    const mo = ov.createDiv({attr:{style:"width:min(920px,96vw);height:90vh;display:flex;flex-direction:column;padding:0;overflow:hidden;border-radius:16px;background:#1e1e1e;border:1px solid rgba(255,255,255,0.08);box-shadow:0 30px 80px rgba(0,0,0,0.7);flex-shrink:0;"}});
    const cl = () => ov.remove();
    ov.onclick = e => { if(e.target===ov) cl(); };

    // -- HEADER ----------------
    const hdr = mo.createDiv({attr:{style:"display:flex;align-items:center;gap:10px;padding:14px 18px;border-bottom:1px solid rgba(255,255,255,0.06);flex-shrink:0;"}});
    ...
    const closeBtn = hdr.createEl("button", {attr:{style:"font-size:0.75em;opacity:0.3;cursor:pointer;padding:3px 8px;border-radius:6px;background:transparent;border:none;color:#fff;transition:all 0.2s;"}});
    closeBtn.innerText = "✕";
    closeBtn.onmouseenter = () => { closeBtn.style.opacity="1"; closeBtn.style.background="rgba(255,85,85,0.15)"; closeBtn.style.color="#ff5555"; };
    closeBtn.onmouseleave = () => { closeBtn.style.opacity="0.3"; closeBtn.style.background="transparent"; closeBtn.style.color="#fff"; };
    closeBtn.onclick = cl;

    // -- BODY ----------------
    const body = mo.createDiv({attr:{style:"flex:1;overflow:hidden;position:relative;background:#525659;"}});
    ...
    const renderWithToken = async () => {
        ...
        try {
            ...
            if(fileMime.includes("google-apps.document") || fileMime.includes("google-apps.spreadsheet")){
                ...
                const scrollArea = body.createDiv({attr:{style:"width:100%;height:100%;overflow-y:auto;padding:28px 36px;box-sizing:border-box;background:#fff;color:#111;font-family:Georgia,serif;font-size:0.93em;line-height:1.7;"}});
            ...
            if(isPDF || fileMime.includes("pdf")){
                ...
                const scrollArea = body.createDiv({attr:{style:"width:100%;height:100%;overflow-y:auto;padding:28px 36px;box-sizing:border-box;background:#fff;color:#111;font-family:Georgia,serif;font-size:0.95em;line-height:1.7;"}});
...
if(!pdfjsLoaded) throw new Error("Could not load PDF renderer — check Obsidian network settings allow external scripts.");
        ...
        const scrollArea = body.createDiv({attr:{style:"width:100%;height:100%;overflow-y:auto;overflow-x:hidden;padding:20px 24px;box-sizing:border-box;display:flex;flex-direction:column;align-items:center;gap:16px;background:#1a1a1a;"}});
...
// -- LEFT DRAG -----------------
const cardBuilders={clock:buildClockCard,objective:buildObjectiveCard,system:buildSystemCard};
...
// -- RIGHT DRAG ----------------
const rightBuilders={pipeline:buildPipelineCard,ctf:buildCTFCard,links:buildLinksCard};
let rightHeightSyncRaf=null;

function syncRightCardHeights(){
    if(!rightCol)return;
    const pipelineCard=rightCol.querySelector('[data-right-id="pipeline"]');
    const ctfCard=rightCol.querySelector('[data-right-id="ctf"]');
    if(!pipelineCard||!ctfCard)return;

    ctfCard.style.minHeight="";
    const pipelineHeight=Math.ceil(pipelineCard.getBoundingClientRect().height);
    if(pipelineHeight>0){
        ctfCard.style.minHeight=`${pipelineHeight}px`;
    }
}
...
function renderGroupHeader(parent,label,color,count){
    const hdr=parent.createDiv({attr:{style:"display:flex;align-items:center;gap:8px;margin:18px 0 8px;padding-bottom:7px;border-bottom:1px solid rgba(255,255,255,0.05);"}});
    hdr.createDiv({attr:{style:`width:3px;height:14px;border-radius:2px;background:${color};flex-shrink:0;`}});
    hdr.createDiv({text:label,attr:{style:`font-size:0.7em;font-weight:800;letter-spacing:2px;color:${color};`}});
...
const AI_SUMMARY_BOX_STYLE="font-size:0.74em;opacity:0.75;line-height:1.55;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:10px;padding:10px 12px;margin-top:8px;max-height:220px;overflow-y:auto;white-space:normal;";
...
function renderEventCard(parent,ev,done){
    ...
    card.onclick=async e=>{
        ...
        const ov2=document.body.createDiv({cls:"zos-modal-overlay"});
        const mo2=ov2.createDiv({cls:"zos-modal",attr:{style:"width:460px;"}});
        const cl2=()=>ov2.remove();
        ov2.onclick=e2=>{if(e2.target===ov2)cl2();};
        mo2.createDiv({cls:"zos-modal-title",text:title});
        ...
        meta.createDiv({attr:{style:"font-size:0.72em;opacity:0.4;font-weight:600;"}}).innerText=calName;
        if(due)meta.createDiv({attr:{style:"font-size:0.7em;opacity:0.3;"}}).innerText=fmtDate(due);
        const rawDesc=(ev.description||"").replace(/https:\/\/classroom\.google\.com\/[^\s<>"\\]*/g,"").replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim();
        if(rawDesc)mo2.createDiv({attr:{style:"font-size:0.82em;opacity:0.7;line-height:1.6;background:rgba(255,255,255,0.03);border-radius:10px;padding:12px;margin-bottom:16px;border:1px solid rgba(255,255,255,0.05);max-height:120px;overflow-y:auto;white-space:pre-wrap;"}}).innerText=rawDesc;

        // -- NOTE ----------------------------
        const noteSection=mo2.createDiv({attr:{style:"margin-bottom:12px;"}});
        noteSection.createDiv({attr:{style:"font-size:0.65em;font-weight:700;letter-spacing:1px;opacity:0.35;margin-bottom:6px;"}}).innerText="NOTE";
        const noteInput=noteSection.createEl("textarea",{attr:{rows:"2",placeholder:"Add a note...",style:"width:100%;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:10px 12px;color:#fff;font-family:var(--zos-font-main);font-size:0.8em;outline:none;resize:none;box-sizing:border-box;transition:border-color 0.2s;"}});
        noteInput.value=getNote(ev);
        noteInput.onfocus=()=>{noteInput.style.borderColor="var(--zos-accent)";};
        noteInput.onblur=()=>{noteInput.style.borderColor="rgba(255,255,255,0.08)";setNote(ev,noteInput.value);renderListView();};

        // -- MATERIALS ---------------
        const attachSection=mo2.createDiv({attr:{style:"margin-bottom:16px;"}});
        attachSection.createDiv({attr:{style:"font-size:0.65em;font-weight:700;letter-spacing:1px;opacity:0.35;margin-bottom:8px;"}}).innerText="MATERIALS";
        ...
        (async()=>{
            try{
                ...
                d.materials.forEach(mat=>{
                    ...
                    const row=attachList.createDiv({attr:{style:"display:flex;align-items:center;gap:10px;padding:9px 12px;background:rgba(255,255,255,0.03);border-radius:10px;border:1px solid rgba(255,255,255,0.06);cursor:pointer;transition:all 0.2s;"}});
                    ...
                    row.onmouseenter=()=>{row.style.background="rgba(255,255,255,0.06)";row.style.borderColor=`${tc}44`;};
        ...
        const prioBtn=btnRow.createEl("button",{attr:{style:`flex:1;font-size:0.75em;font-weight:700;padding:8px;border-radius:9px;background:${curPrio==="HIGH"?"rgba(180,80,255,0.15)":"rgba(255,255,255,0.05)"};color:${curPrio==="HIGH"?"#b450ff":"rgba(255,255,255,0.5)"};border:1px solid ${curPrio==="HIGH"?"rgba(180,80,255,0.3)":"rgba(255,255,255,0.08)"};cursor:pointer;transition:all 0.2s;`}});
        ...
        prioBtn.onclick=()=>{
            ...
            prioBtn.style.background=newPrio==="HIGH"?"rgba(180,80,255,0.15)":"rgba(255,255,255,0.05)";
            prioBtn.style.color=newPrio==="HIGH"?"#b450ff":"rgba(255,255,255,0.5)";
            prioBtn.style.borderColor=newPrio==="HIGH"?"rgba(180,80,255,0.3)":"rgba(255,255,255,0.08)";
            renderSummaryBar();renderListView();
        };
        if(lnk){
        ...
        // -- LINKED NOTE -----------------
        const noteLinkKey=`zos-note-link-${ev.id}`;
        const linkedNotePath=localStorage.getItem(noteLinkKey);
        const noteLinkRow=mo2.createDiv({attr:{style:"margin-bottom:8px;"}});

        const renderNoteLinkBtn=()=>{
            noteLinkRow.innerHTML="";
            const existingPath=localStorage.getItem(noteLinkKey);
            if(existingPath){
                const noteFile=app.vault.getAbstractFileByPath(existingPath);
                const btnRow2=noteLinkRow.createDiv({attr:{style:"display:flex;gap:8px;"}});
                const openBtn=btnRow2.createEl("button",{attr:{style:`flex:1;font-size:0.75em;font-weight:700;padding:8px;border-radius:9px;background:rgba(var(--zos-accent-rgb),0.1);color:var(--zos-accent);border:1px solid rgba(var(--zos-accent-rgb),0.2);cursor:pointer;`}});
                openBtn.innerText=`📄 ${existingPath.split("/").pop()}`;
                openBtn.onclick=()=>{app.workspace.openLinkText(existingPath,"",false);cl2();};
                const unlinkBtn=btnRow2.createEl("button",{attr:{style:`font-size:0.75em;font-weight:700;padding:8px 12px;border-radius:9px;background:rgba(255,85,85,0.08);color:rgba(255,85,85,0.6);border:1px solid rgba(255,85,85,0.15);cursor:pointer;`}});
                unlinkBtn.innerText="✕";
                unlinkBtn.onclick=()=>{localStorage.removeItem(noteLinkKey);renderNoteLinkBtn();};
            } else {
                const linkBtn=noteLinkRow.createEl("button",{attr:{style:`width:100%;font-size:0.75em;font-weight:700;padding:8px;border-radius:9px;background:rgba(255,255,255,0.04);color:rgba(255,255,255,0.4);border:1px dashed rgba(255,255,255,0.12);cursor:pointer;transition:all 0.2s;`}});
                linkBtn.innerText="📎 Link a Note";
                linkBtn.onmouseenter=()=>{linkBtn.style.borderColor="var(--zos-accent)";linkBtn.style.color="var(--zos-accent)";};
                linkBtn.onmouseleave=()=>{linkBtn.style.borderColor="rgba(255,255,255,0.12)";linkBtn.style.color="rgba(255,255,255,0.4)";};
                linkBtn.onclick=async()=>{
                    // File browser to pick folder, then create note
                    let currentPath="/";
                    const fb=document.body.createDiv({cls:"zos-modal-overlay"});
                    const fm=fb.createDiv({cls:"zos-modal",attr:{style:"width:480px;max-height:80vh;display:flex;flex-direction:column;"}});
                    fb.onclick=e=>{if(e.target===fb)fb.remove();};
                    fm.createDiv({cls:"zos-modal-title",text:"CHOOSE FOLDER FOR NOTE"});
                    const noteNameRow=fm.createDiv({attr:{style:"margin-bottom:12px;"}});
                    noteNameRow.createDiv({attr:{style:"font-size:0.65em;font-weight:700;letter-spacing:1px;opacity:0.35;margin-bottom:6px;"}}).innerText="NOTE NAME";
                    const noteNameInp=noteNameRow.createEl("input",{cls:"zos-modal-input",attr:{type:"text",value:title,style:"margin-bottom:0;"}});
                    noteNameInp.focus();noteNameInp.select();
                    noteNameInp.onfocus=()=>{noteNameInp.style.borderColor="var(--zos-accent)";};
                    noteNameInp.onblur=()=>{noteNameInp.style.borderColor="rgba(255,255,255,0.1)";};
                    const bcEl=fm.createDiv({attr:{style:"display:flex;align-items:center;gap:4px;flex-wrap:wrap;padding:8px 12px;background:rgba(255,255,255,0.03);border-radius:10px;margin-bottom:10px;border:1px solid rgba(255,255,255,0.06);min-height:34px;"}});
                    const flEl=fm.createDiv({attr:{style:"flex:1;overflow-y:auto;max-height:280px;display:flex;flex-direction:column;gap:4px;padding-right:2px;margin-bottom:14px;"}});
                    const newFolderRow2=fm.createDiv({attr:{style:"display:flex;gap:8px;margin-bottom:14px;"}});
                    const newFolderInp2=newFolderRow2.createEl("input",{attr:{type:"text",placeholder:"New folder name...",style:"flex:1;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:8px 12px;color:#fff;font-size:0.8em;outline:none;"}});
                    const newFolderBtn2=newFolderRow2.createEl("button",{attr:{style:"padding:8px 14px;border-radius:10px;background:rgba(var(--zos-accent-rgb),0.12);color:var(--zos-accent);border:1px solid rgba(var(--zos-accent-rgb),0.25);font-size:0.75em;font-weight:700;cursor:pointer;white-space:nowrap;"}});
                    newFolderBtn2.innerText="+ Create";
                    newFolderInp2.onfocus=()=>{newFolderInp2.style.borderColor="var(--zos-accent)";};
                    newFolderInp2.onblur=()=>{newFolderInp2.style.borderColor="rgba(255,255,255,0.08)";};
                    newFolderBtn2.onclick=async()=>{
                        const name=newFolderInp2.value.trim();if(!name)return;
                        const newPath=currentPath==="/"?name:`${currentPath}/${name}`;
                        try{await app.vault.createFolder(newPath);newFolderInp2.value="";currentPath=newPath;renderFL();}catch(e){new Notice(`Error: ${e.message}`);}
                    };
                    newFolderInp2.onkeydown=e=>{if(e.key==="Enter")newFolderBtn2.click();};

                    const actRow=fm.createDiv({attr:{style:"display:flex;gap:8px;"}});
                    actRow.createEl("button",{cls:"zos-modal-btn zos-btn-cancel",text:"Cancel",attr:{style:"flex:1;"}}).onclick=()=>fb.remove();
                    const saveHereBtn=actRow.createEl("button",{cls:"zos-modal-btn zos-btn-save",attr:{style:"flex:1;"}});
                    saveHereBtn.innerText="💾 Save Here";

                    const renderBC=()=>{
                        bcEl.innerHTML="";
                        if(currentPath==="/"){bcEl.createDiv({attr:{style:"font-size:0.72em;font-weight:700;color:var(--zos-accent);"}}).innerText="Vault Root";return;}
                        const parts=currentPath.split("/");
                        const rc=bcEl.createDiv({attr:{style:"font-size:0.72em;color:rgba(255,255,255,0.4);cursor:pointer;"}});
                        rc.innerText="Root";rc.onclick=()=>{currentPath="/";renderFL();};
                        bcEl.createDiv({attr:{style:"font-size:0.7em;opacity:0.3;"}}).innerText="›";
                        parts.forEach((p,i)=>{
                            const isLast=i===parts.length-1;
                            const c=bcEl.createDiv({attr:{style:`font-size:0.72em;font-weight:${isLast?"700":"400"};color:${isLast?"var(--zos-accent)":"rgba(255,255,255,0.4)"};${isLast?"":"cursor:pointer;"}white-space:nowrap;`}});
                            c.innerText=p;
                            if(!isLast){c.onclick=()=>{currentPath=parts.slice(0,i+1).join("/");renderFL();};}
                    ...
                    const renderFL=()=>{
                        ...
                        if(currentPath!=="/"){
                            const up=flEl.createDiv({attr:{style:"display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:10px;cursor:pointer;color:rgba(255,255,255,0.4);font-size:0.82em;"}});
                            up.createDiv({attr:{style:"width:18px;text-align:center;"}}).innerText="↑";up.createDiv({text:".. (go up)"});
                            up.onmouseenter=()=>{up.style.background="rgba(255,255,255,0.04)";};up.onmouseleave=()=>{up.style.background="";};
                            up.onclick=()=>{const p=currentPath.split("/");currentPath=p.length<=1?"/":p.slice(0,-1).join("/");renderFL();};
                        }
                        if(!dirs.length)flEl.createDiv({attr:{style:"text-align:center;padding:16px;opacity:0.3;font-size:0.78em;"}}).innerText="No subfolders";
                        dirs.forEach(f=>{
                            const row=flEl.createDiv({attr:{style:"display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:10px;cursor:pointer;border:1px solid transparent;"}});
                            row.createDiv({attr:{style:"width:18px;text-align:center;"}}).innerText="📁";
                            row.createDiv({attr:{style:"font-size:0.84em;font-weight:600;flex:1;"}}).innerText=f.name;
                            row.createDiv({attr:{style:"font-size:0.75em;opacity:0.3;"}}).innerText="›";
                            row.onmouseenter=()=>{row.style.background="rgba(255,255,255,0.05)";row.style.borderColor="rgba(255,255,255,0.07)";};
                            row.onmouseleave=()=>{row.style.background="";row.style.borderColor="transparent";};
                            row.onclick=()=>{currentPath=f.path;renderFL();};
                        });
                        const fn=currentPath==="/"?"Vault Root":currentPath.split("/").pop();
                        saveHereBtn.innerText=`💾 Save in "${fn}"`;
                    };
                    saveHereBtn.onclick=async()=>{
                        ...
                        try{
                            ...
                            localStorage.setItem(noteLinkKey,notePath);
                            fb.remove();
                            renderNoteLinkBtn();
                        }catch(e){new Notice(`Error: ${e.message}`);}
                    };
        ...
        const db=mo2.createEl("button",{attr:{style:`width:100%;font-size:0.75em;font-weight:700;padding:8px;border-radius:9px;background:${done?"rgba(255,85,85,0.1)":"rgba(0,255,136,0.08)"};color:${done?"#ff5555":"#00ff88"};border:1px solid ${done?"rgba(255,85,85,0.2)":"rgba(0,255,136,0.2)"};cursor:pointer;margin-bottom:8px;`}});
    ...
    const chk=chkW.createDiv({attr:{style:`width:19px;height:19px;border-radius:5px;border:2px solid ${done?"#00ff88":"rgba(255,255,255,0.18)"};background:${done?"#00ff88":"transparent"};flex-shrink:0;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all 0.18s;font-size:11px;color:#000;`}});
    ...
    chk.onmouseleave=()=>{if(isDone(ev)){chk.innerText="✓";chk.style.background="#00ff88";chk.style.borderColor="#00ff88";chk.style.color="#000";}else{chk.innerText="";chk.style.borderColor="rgba(255,255,255,0.18)";chk.style.background="transparent";}};
    const cnt=card.createDiv({attr:{style:"flex:1;padding:12px 14px 12px 0;min-width:0;"}});
    ...
    r2.createDiv({attr:{style:"font-size:0.7em;opacity:0.45;font-weight:600;"}}).innerText=calName;
    if(due){r2.createDiv({attr:{style:"font-size:0.62em;opacity:0.25;"}}).innerText="·";r2.createDiv({attr:{style:"font-size:0.67em;opacity:0.35;"}}).innerText=fmtDate(due);}
    if(note)cnt.createDiv({attr:{style:"font-size:0.7em;opacity:0.4;margin-top:4px;font-style:italic;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"}}).innerText=`✎ ${note}`;
...
function renderMonth(){
    ...
    for(let day=1;day<=daysInMonth;day++){
        ...
        let bdr=isToday?"var(--zos-accent)":"rgba(255,255,255,0.04)";
        ...
        if(dayEvs.length>0){
            ...
            cell.onmouseenter=()=>{cell.style.background=isToday?"rgba(var(--zos-accent-rgb),0.22)":"rgba(255,255,255,0.05)";cell.style.borderColor=dotC;};
            ...
            cell.onclick=()=>{
                const showDesc=(ev)=>{
                    ...
                    mo2.createDiv({cls:"zos-modal-title",text:title2});
                    const meta=mo2.createDiv({attr:{style:"display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:16px;"}});
                    ...
                    meta.createDiv({attr:{style:"font-size:0.72em;opacity:0.4;font-weight:600;"}}).innerText=getCalName(ev);
                    if(due2)meta.createDiv({attr:{style:"font-size:0.7em;opacity:0.3;"}}).innerText=fmtDate(due2);
                    const rawDesc=(ev.description||"").replace(/https:\/\/classroom\.google\.com\/[^\s<>"\\]*/g,"").replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim();
                    if(rawDesc){mo2.createDiv({attr:{style:"font-size:0.82em;opacity:0.7;line-height:1.6;background:rgba(255,255,255,0.03);border-radius:10px;padding:12px;margin-bottom:16px;border:1px solid rgba(255,255,255,0.05);max-height:160px;overflow-y:auto;white-space:pre-wrap;"}}).innerText=rawDesc;}
                    else{mo2.createDiv({attr:{style:"font-size:0.8em;opacity:0.3;text-align:center;padding:20px;margin-bottom:16px;"}}).innerText="No description available.";}
                    const note=getNote(ev);if(note)mo2.createDiv({attr:{style:"font-size:0.75em;opacity:0.5;font-style:italic;margin-bottom:14px;"}}).innerText=`✎ ${note}`;
                    ...
                    const db=btnRow.createEl("button",{attr:{style:`flex:1;font-size:0.75em;font-weight:700;padding:8px;border-radius:9px;background:${evDone?"rgba(255,85,85,0.1)":"rgba(0,255,136,0.08)"};color:${evDone?"#ff5555":"#00ff88"};border:1px solid ${evDone?"rgba(255,85,85,0.2)":"rgba(0,255,136,0.2)"};cursor:pointer;`}});
                    db.innerText=evDone?"↩ Undo":"✓ Done";db.onclick=()=>{toggleDone(ev);renderHeaderStats();renderSummaryBar();renderMonth();cl2();};
                    mo2.createEl("button",{cls:"zos-modal-btn zos-btn-cancel",text:"Close",attr:{style:"width:100%;margin-top:8px;"}}).onclick=cl2;
                };

                if(dayEvs.length===1){showDesc(dayEvs[0]);}
                else{
                    const ov2=document.body.createDiv({cls:"zos-modal-overlay"});
                    const mo2=ov2.createDiv({cls:"zos-modal",attr:{style:"width:420px;"}});
                    mo2.createDiv({cls:"zos-modal-title",text:cd.toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})});
                    const cl2=()=>ov2.remove();ov2.onclick=e2=>{if(e2.target===ov2)cl2();};
                    mo2.createDiv({attr:{style:"font-size:0.72em;opacity:0.4;margin-bottom:12px;"}}).innerText="Click an assignment to see its description.";
                    dayEvs.forEach(ev=>{
